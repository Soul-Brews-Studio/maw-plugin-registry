...
export async function cmdBuddy(oracle: string, opts: BuddyOptions): Promise<void> {
  ...
  console.log(`\x1b[36m→\x1b[0m pair:     ${engineA} (A) × ${engineB} (B)`);
  ...
  if (opts.dryRun) {
    ...
    console.log(`  3. prime both via maw hey`);
    return;
  }
  ...
  const primingA = buildBuddyPriming({
    yourName: nameA, yourPane: nameA, buddyName: nameB, buddyPane: nameB,
    task: opts.task, taskSlug, contextPath: `ψ/inbox/buddy/${taskSlug}.md`,
    worktreePath, lineage, yourRole: opts.roleA ?? "spec", buddyRole: opts.roleB ?? "impl", leadPane,
  });
  const primingB = buildBuddyPriming({
    yourName: nameB, yourPane: nameB, buddyName: nameA, buddyPane: nameA,
    task: opts.task, taskSlug, contextPath: `ψ/inbox/buddy/${taskSlug}.md`,
    worktreePath, lineage, yourRole: opts.roleB ?? "impl", buddyRole: opts.roleA ?? "spec", leadPane,
  });
